...
void Asn1::Update(double dt)
{
	...
	if (gs == STAGE_1_CLEARED)
	{
		if (Application::IsKeyPressed(VK_RETURN))
			gs = STAGE2;
		return;
	}
	if (gs == STAGE_2_CLEARED)
	{
		if (Application::IsKeyPressed(VK_RETURN))
			gs = STAGEFINAL;
		return;
	 ...
	 if (gs == STAGE1)
	{
		if (enemySpawnTimer > 1.f)
		{
			...
			go->scale.Set(6, 6, 1);
			go->pos.Set(Math::RandFloatMinMax(0, m_worldWidth), Math::RandFloatMinMax(0, m_worldHeight), asteroidZ);
	 ...
	 if (gs == STAGEFINAL)
	{
		...
		if (bossFireDebounce > 1.f / bossFireRate)
		{
	...
	if (Application::IsKeyPressed('Z') && firingDebounce > 1.f / fireRate)
	{
		...
		GameObject* bulletLeft = FetchGO();
		bulletLeft->type = GameObject::GO_BULLET;
		bulletLeft->scale = bulletSize;
		bulletLeft->pos = m_ship->pos;

		GameObject* bulletRight = FetchGO();
		bulletRight->type = GameObject::GO_BULLET;
		bulletRight->scale = bulletSize;
		bulletRight->pos = m_ship->pos;
		...
		if (m_ship->vel.IsZero())
		{
			...
			bulletRotation.SetToRotation(100, 0, 0, 1);
			bulletLeft->vel = (shipRotation * bulletRotation) * Vector3(30, 0, 0);

			bulletRotation.SetToRotation(80, 0, 0, 1);
			bulletRight->vel = (shipRotation * bulletRotation) * Vector3(30, 0, 0);
		}
		else
		{
			...
			bulletRotation.SetToRotation(10, 0, 0, 1);
			bulletLeft->vel = (shipRotation * bulletRotation) * direction * BULLET_SPEED;

			bulletRotation.SetToRotation(-10, 0, 0, 1);
			bulletRight->vel = (shipRotation * bulletRotation) * direction * BULLET_SPEED;
		}
	}
	//Mouse Section
...
void Asn1::RenderGO(GameObject *go)
{
	switch (go->type)
	{
...
void Asn1::Render()
{
	...
	else
	{
		...
		// Stage info
		ss.str("");
		if (gs == STAGE1 || gs == STAGE2 || gs == STAGEFINAL)
		{
			ss << "Current Stage: ";
			if (gs == STAGE1)
				ss << "Stage 1";
				...
				ss << "Final Stage";
			RenderTextOnScreen(meshList[GEO_TEXT], ss.str(), Color(1, 1, 0), 3, 23, 57);
		}
