...
void SceneKinematics::Update(double dt)
{
	...
	{
		//Exercise 6: adjust simulation speed
		m_speed = Math::Min(0.f, m_speed - 0.1f);
	}
	...
	if(Application::IsKeyPressed(VK_SPACE))
	{
		...
		for (auto go : m_goList)
		{
			if (!go->active)
			{
				...
				go->vel.Set(Math::RandFloatMinMax(-30, 30), Math::RandFloatMinMax(-30, 30), 0);
				if (++count == 10)
				{
					break;
				}
	...
	if(Application::IsKeyPressed('V'))
	{
		...
		for (auto q : m_goList)
		{
			if (!q->active)
			{
				...
				q->pos.Set(Math::RandFloatMinMax(0.f, m_worldWidth), Math::RandFloatMinMax(0.f, m_worldHeight), 0);
				if (++count == 10)
				{
					break;
				}
	...
	if(!bLButtonState && Application::IsMousePressed(0))
	{
		...
		if (!m_ghost->active)
		{
			...
			m_ghost->pos.Set(worldX, worldY, 0.f);
		}
	...
	else if(bLButtonState && !Application::IsMousePressed(0))
	{
		...
		for (std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
		{
			...
			if (!go->active)
			{
				...
				go->vel.Set(20, 20, 0);*/
				(*it)->pos.Set(m_ghost->pos.x, m_ghost->pos.y, 0.f);
				(*it)->vel.Set(m_ghost->pos.x - worldX, m_ghost->pos.y - worldY, 0.f);
				m_ghost->vel = (*it)->vel;
		...
		//s = u * t + 0.5 * a * t * t
		//(0.5 * a) * t * t + (u) * t + (-s) = 0
		m_timeEstimated2 = (-u + sqrt(u * u - 2 * a * (-s))) / a;
	...
	for(std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
	{
		...
		if(go->active)
		{
			if(go->type == GameObject::GO_BALL)
			{
				...
				Vector3 dv = m_gravity * m_speed * dt;
				go->vel += dv;
				Vector3 ds = go->vel * m_speed * dt;
				go->pos += ds;

				go->vel.Set((go->vel.x + go->vel.x + m_gravity.x * (float)dt) / 2.f,
					(go->vel.y + go->vel.y + m_gravity.y * (float)dt) / 2.f,
					0.f);
				go->pos.x += go->vel.x * (float)dt;
				go->pos.y += go->vel.y * (float)dt;
			...
			if (go->pos.x > m_worldWidth + 2 || go->pos.x < -2 || go->pos.y > m_worldHeight + 2 || go->pos.y <= -2)
			{
				go->active = false;
			}
...
void SceneKinematics::Render()
{
	...
	ss3.precision(5);
	ss3 << "Ghost Velocity: " << m_ghost->vel;
	RenderTextOnScreen(meshList[GEO_TEXT], ss3.str(), Color(0, 1, 0), 3, 0, 9);

	std::ostringstream ss4;
	ss4.precision(5);
	ss4 << "Ghost Position: " << m_ghost->pos;
	RenderTextOnScreen(meshList[GEO_TEXT], ss4.str(), Color(0, 1, 0), 3, 0, 12);

	//Exercise 11: print kinematics information
	std::ostringstream ss5;
	ss5.precision(5);
	ss5 << "Time Estimated 1: " << m_timeEstimated1;
	RenderTextOnScreen(meshList[GEO_TEXT], ss5.str(), Color(0, 1, 0), 3, 0, 21);

	std::ostringstream ss6;
	ss6.precision(5);
	ss6 << "Time Estimated 2: " << m_timeEstimated2;
	RenderTextOnScreen(meshList[GEO_TEXT], ss6.str(), Color(0, 1, 0), 3, 0, 18);

	std::ostringstream ss7;
	ss7.precision(5);
	ss7 << "Height Estimated: " << m_heightEstimated;
	RenderTextOnScreen(meshList[GEO_TEXT], ss7.str(), Color(0, 1, 0), 3, 0, 15);

	std::ostringstream ss8;
	ss8.precision(5);
	ss8 << "Time Taken 1: " << m_timeTaken1;
	RenderTextOnScreen(meshList[GEO_TEXT], ss8.str(), Color(0, 1, 0), 3, 0, 30);

	std::ostringstream ss9;
	ss9.precision(5);
	ss9 << "Time Taken 2: " << m_timeTaken2;
	RenderTextOnScreen(meshList[GEO_TEXT], ss9.str(), Color(0, 1, 0), 3, 0, 27);

	std::ostringstream ss10;
	ss10.precision(5);
	ss10 << "Max Height: " << m_heightMax;
	RenderTextOnScreen(meshList[GEO_TEXT], ss10.str(), Color(0, 1, 0), 3, 0, 24);
